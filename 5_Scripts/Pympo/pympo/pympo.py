...
# Algorithm and parameters taken from papers Lian2010, Weinans1992,
# Mullender1993
#
# AUTHOR:           Bruno Luna
#
...
# === Meshing
mapdl.et(1, "PLANE182")  # Define the local element type 1 as PLANE182
mapdl.keyopt(
    1, 1, 2
...
mapdl.smrtsize(mesh_refine)  # Global mesh ref. level (1(fine) to 10(rough))
...
for el in range(nelem):
    ...
    mapdl.emodif(el_ansys, "MAT", el_ansys)  # Assign mat type
...
for iter in range(1, niter + 1):  # Loop over remodeling steps
    ...
    for el in range(nelem):  # Loop over all elements
        el_ansys = el + 1
        AR_ActVal[el] = mapdl.get(
            "AR_ActVal", "ELEM", el_ansys, "ETAB", "et_SED"
    ...
    for el in range(nelem):  # Loop over all elements
        ...
        AR_EMod[el] = mapdl.get(
            "AR_EMod", "EX", mat_no
        )  # Get Young's modulus of that material number
        AR_EMod[el] = cC * (AR_Rho[el] ** gC)  # Change Young modulus
        mapdl.mp("EX", el_ansys, AR_EMod[el])  # Set Young modulus to element
# ===============================================================================
# END OF LOOP
# ===============================================================================
